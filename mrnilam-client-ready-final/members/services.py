...
def normalize_phone(value):
    """Normalize explicit country codes without guessing ambiguous local numbers."""
    ...
    if international and 8 <= len(digits) <= 15:
        return f"+{digits}"
    if digits.startswith("977") and 10 <= len(digits) <= 13:
        return f"+{digits}"
    if digits.startswith("91") and len(digits) == 12:
        return f"+{digits}"
    if len(digits) == 10 and digits.startswith(("97", "98")):
        return f"+977{digits}"
    # A ten-digit value starting 6-9 may be an Indian number or another local
    # format. Preserve it rather than attaching an unverified country code.
    return digits
...
def get_or_create_unit(level, name_en, name_ne=""):
    ...
    clean_name_ne = str(name_ne or KNOWN_UNIT_NAMES_NE.get(clean_name, "")).strip()
    unit, created = OrganizationUnit.objects.get_or_create(
        level=level or "unknown",
        name_en=clean_name,
        defaults={"name_ne": clean_name_ne},
    )
    if not created and clean_name_ne and not unit.name_ne:
        unit.name_ne = clean_name_ne
...
def sync_legacy_member(legacy_member):
    ...
    record.status = "active" if legacy_member.status in {"", "unknown", None} else legacy_member.status
